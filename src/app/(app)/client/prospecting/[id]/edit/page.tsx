...
export default async function EditProspectingClient({
  params,
}: {
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <PageHeader
        title={`Editar ${c.name}`}
        description="Ajuste os parâmetros. A chave da HighLevel fica apenas se você digitar uma nova."
      />
      <ProspectingWizard
        mode="edit"
        scope="client"
        tenants={tenants}
        cancelHref={`/client/prospecting/${id}`}
        campaignId={id}
        initial={{
          tenantId: c.tenantId,
          name: c.name,
